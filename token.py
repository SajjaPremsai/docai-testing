...
def get_installation_token(installation_id: str, app_id: str) -> str:
    ...
    # This JWT authenticates the GitHub App itself using its private key
    # and is required to request an installation access token.
    ...
    # The GitHub API documentation specifies this endpoint for creating installation tokens.
    url = f"https://api.github.com/app/installations/{installation_id}/access_tokens"

    # 3. Prepare the request headers.
    # - "Authorization": Uses the generated JWT prefixed with "Bearer" to authenticate
    #                    the GitHub App.
    # - "Accept": Specifies the desired GitHub API version and format (recommended
    #             by GitHub for future compatibility and specific features).
    headers = {
    ...
    # that grants permissions based on the app's configuration and installation's scope.
    response = requests.post(url, headers=headers)

    # 5. Check for HTTP errors.
    # If the request was not successful (e.g., 401 Unauthorized, 403 Forbidden, 404 Not Found,
    # or any other non-2xx status code), this will raise an `HTTPError`.
    response.raise_for_status()

    # 6. Parse the JSON response and extract the 'token'.
    # The response body typically contains details such as the installation token,
    # its expiration time, and the permissions granted. We specifically need the 'token' value.
